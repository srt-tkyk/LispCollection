...
餌に触れればエネルギーが増え、動けば減る。尽きれば死に、一定量を超えれば分裂する。

世界の側の規則——餌の再生量、繁殖の閾値、攻撃性がいくつを超えたら捕食者になるか、
個体数の上限——は人間が決めている（§6 に一覧）。**決めていないのは、各個体が
1 tick に旋回量と速度をどう計算するか、その1点だけである。**

計算するのはゲノムが持つ2本のS式の木 `turn-tree` / `speed-tree` で、
入力は `food-angle`（最寄りの餌への相対角）`food-dist`（正規化距離）
`energy` `age` `noise` の5つ。**この木の形そのものが、変異と交叉で書き換わる。**
...
木に対する操作は3つだけである。変異は部分木の書き換え、交叉は部分木の交換、
表現型化は `COMPILE` によるネイティブコード化。数値ベクトルを揺らす代わりに、
**プログラムの構造そのものを書き換えている。**
