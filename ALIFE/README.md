...
500×500 のトーラス世界に個体と餌を撒く。個体は 1 tick ごとに向きと速度を決めて動き、
餌に触れればエネルギーが増え、動けば減る。**尽きれば死に、貯まれば分裂する。**
与えた規則はこれだけで、あとは放っておく。

そこから出てくるもの:

- **餌に向かう行動** — 「餌の方を向け」とはどこにも書いていないのに、
  1200 tick で全個体が餌の方向を参照するようになる（§7.1）
- **捕食者への種分化** — 攻撃性が閾値を超えた個体が `change-class` で肉食に転じ、
  草食個体を襲いはじめる。可逆で、草食に戻る個体もいる（§7.3）
- **個体数の振動** — 捕食者が増えれば獲物が枯れ、捕食者が飢えて減り、また獲物が回復する。
  この周期を書いた覚えはない（冒頭の動画で赤い個体が増えていくのがそれ）

**ふつうの進化シミュレーションと違うのは、個体の行動がプログラムに書かれていないこと。**
「餌を見つけたら向きを変えて近づく」に相当するコードが存在しない。
どう動くかは、各個体がゲノムとして持つ **S式の木**が決める。
突然変異は部分木の書き換え、交叉は部分木の交換、表現型化は `COMPILE` による
ネイティブコード化。遺伝子とソースコードが同じ表現なので、変換器がほとんど要らない。

その帰結として、**進化した個体の「脳」はそのまま Lisp のソースコードとして読める。**
