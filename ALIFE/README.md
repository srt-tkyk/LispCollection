...
餌に向かう行動を書いたコードは存在しないので、**この2行は進化が見つけたものである。**
...
この種のシミュレーションで標準的なのは、ゲノムを固定長の数値ベクトルにする設計である。
たとえばこう書く。
...
```lisp
(if food (steer-toward org food turn-rate) (wander org turn-rate))
```
...
変異は5つの数値にガウスノイズを足すだけで、行動を書いた関数の中身を一文字も変えない。
...
変異は ℝ⁵ の点を動かすだけで φ 自体を動かさないので、
進化は像の上を登ることしかできない。
これは実装の粗さではなく、**固定次元ゲノムという設計の帰結**である。
...
1200 tick までに一匹残らず淘汰された。感覚入力と運動出力の結合が、
それを書いた関数なしに獲得されている。固定次元GAでは原理的に動かない量である。
