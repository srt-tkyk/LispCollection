...
- **餌に向かう行動** — 「餌の方を向け」とはどこにも書いていないのに、
  1200 tick で全個体が餌の方向を参照するようになる（§8.1）
- **捕食者への種分化** — 攻撃性が閾値を超えた個体が `change-class` で肉食に転じ、
  草食個体を襲いはじめる。可逆で、草食に戻る個体もいる（§8.3）
...
「受動的な**記述**」と「能動的な**構成器**」の対が、Lisp では `quote` と `eval` に
そのまま対応する。**同一のリストが、変異の対象であり、実行される脳であり、
保存形式であり、人間が読める記述でもある。**（§5）
...
ここまでの仕掛けは、原理的には他の言語でも作れる。
それでもこれを Lisp で書く理由を述べる。
...
Lisp の `quote` と `eval` が、まさにこの対をなす。

```lisp
(defparameter *g* '(if> food-dist 0.5 food-angle noise))
;; 「餌が遠ければ餌の方を向き、近ければ適当に曲がる」

*g*                                  ; データ — 変異・交叉が書き換える対象
(tree-size *g*)                      ; => 5

(funcall (brain-fn *g*) 1.23 0.8 0.5 0.2)   ; => 1.23   実行 — 表現型としての振る舞い
(funcall (brain-fn *g*) 1.23 0.2 0.5 0.2)   ; => 0.2589 （こちらは noise 側）
```

同一のリストが、遺伝子型でもあり表現型の記述でもある。
この一致は Lisp の設計思想（homoiconicity）から直接くるもので、
...
そして同じ設計なら Python でも C でも実装は変わらない。
**この段階では、Lisp を使う理由が何もない。**

S式にすると空間の形が変わる。

| | 遺伝子型空間 | 次元 |
|---|---|---|
| 固定次元 | G = ℝ⁵ | 永遠に 5。φ は固定された1本の射 |
| S式 | G = 自由 Σ 代数 T_Σ(X) | 上限なし。サイズ n の項数は指数的に増える |

探索しているのは「プリミティブ集合 Σ の合成閉包」である。
Σ が有限でも閉包は非有界なので、**次元という概念が消える**。
これが「今まで存在しなかった行動」の出どころで、
§8.1 や §8.4 で見ているのはその結果にほかならない。
...
§4.4 の `emit` が2例外を除いてほぼ恒等写像になるのは、この一致の直接の現れである。

**Python は前半（実行時生成）だけ、C は後半（ネイティブ速度）だけを持っている。**

### 5.4 主張の限界

S式ゲノムにしても、**自動的に何でも進化するわけではない。**

プリミティブ集合 Σ は結局人間が手で選ぶ。内部状態を持つ部品を入れなければ記憶は
進化しないし、他個体を参照する述語がなければ群れも出ない。この実装にはどちらも
入っていないので、記憶に依存する戦略も群れ行動も**原理的に出現しない**（§10）。

つまり、固定次元では「行動」を決め打ちしていたのが、
S式では「**語彙**」を決め打ちすることに変わっただけ、とも言える。
